	...
	function revelaMinas(){
		$("table#mitabla tr td").each(function (){
			...
			if($(this).is(":contains('94')")){
				...
				$(this).html("<img src=\"minegame.png\" width = '18' height = '18'>");
				}	
		});
		var confirmacion = confirm("Deseas reiniciar el juego?");
			if (confirmacion){
	...
	function bindings(){
		...
		$("table#mitabla tr td").bind("click",function(){
		 ...
		 if ($(this).hasClass("tapa")){
			...
			else if ($(this).is(":contains('73')")){
			$(this).addClass("siete");
			$(this).html(7);
			}
			else if ($(this).is(":contains('84')")){
			$(this).addClass("ocho");
			$(this).html(8);
			}
			...
			if (total == 0){
			var confirmacion = confirm("Ganaste!!! Quieres jugar otra vez??");
			if (confirmacion){
				window.location.reload(true);
			}
			else {
